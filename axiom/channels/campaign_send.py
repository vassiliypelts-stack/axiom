...
def _team(cid: int) -> list[dict]:
    ...
    with database.get_conn() as conn:
        rows = conn.execute(
            ...
            # Тихие номера — первыми в очереди. Заход берёт первого с остатком нормы
            # (_pick, rr=0), и 25.09.2026 каждый заход уходил на старый номер, у
            # которого только что истекла пауза: он тут же снова ловил PeerFlood,
            # отправлено 0, а стук до очереди не доходил. Норма у всех своя, так что
            # старые пойдут следом, когда тихие её выберут.
            "ORDER BY quiet DESC, a.id",
...
# 23.09.2026 порог снижен 14 -> 13 по решению владельца: шесть номеров команды
# 9407 закончили прогрев (14/14 ступеней), сессии живы, прокси рабочие, старые
# паузы истекли 19.09 — ждать ровно сутки ради формальной цифры смысла нет.
...
async def run(cid: int, limit: int, test: bool = False,
    ...
    if team:
        ...
        for acc in team:
            ...
            # Тихий номер (quiet) шлёт только стук «Добрый день, Татьяна?» — его и
            # задумывали для молодых номеров (warmup: стук идёт с первой ступени
            # прогрева), поэтому возрастной порог к нему не применяем. 25.09.2026,
            # решение владельца: партия 17-18.09 стучится сразу, не ждёт 13 дней.
            if not test and not acc.get("quiet"):
                try:
    ...
    for row in rows:
            ...
            if cat == "spam":
                ...
                if s["id"]:
                    with database.get_conn() as conn:
                        ...
                        # ПАУЗА В ЧАСАХ, А НЕ В СУТКАХ.
                        #
                        # Было min(n, 7) ДНЕЙ: второй PeerFlood = двое суток простоя,
                        # третий — трое. Для взрослого номера это несоразмерно: он
                        # отправляет 1-2 письма, получает придержку и выпадает из работы
                        # на дни. 19.09 по кампании 9407 четыре номера не слали с 17-го,
                        # а пауза всё ещё висела — при живой команде кампания стояла.
                        #
                        # Telegram придерживает отправку на часы, а не на дни: PeerFlood
                        # снимается сам, и ежедневный лимит на номер всё равно держит
                        # темп (квота 2 в сутки у 24-дневных). Поэтому ступени теперь
                        # 4 → 8 → 12 … часов с потолком в сутки: номер возвращается в
                        # работу в тот же день и шлёт свои 1-2 письма, а не простаивает.
                        pause_hours = min(4 * n, 24)
                        # РАЗБРОС ВОЗВРАТА. Одинаковая пауза возвращает всю команду в
                        # строй почти одновременно: 22.09 восемь отправителей 9407
                        # слегли за три часа и должны были выйти в окно 07:04-10:49 —
                        # то есть снова начать писать незнакомцам синхронно, тем же
                        # почерком, за который их и придержали. Добавляем каждому свой
                        # случайный сдвиг, чтобы номера выходили вразнобой.
                        pause_hours += random.randint(0, 6)
                        ...
                        database.add_event(conn, "ban", f"⚠ PeerFlood: «{s['label']}»",
                                           ...
                                           f"рассылки на {pause_hours} ч. Снимется само; чаще одного "
        ...
        if sent < cap:
            if test:
                ...
                # Антибан здесь не при чём — спамом себе быть нельзя. Потолок 10 сек:
                # проверка нескольких номеров подряд должна укладываться в минуту.
