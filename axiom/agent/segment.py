...
# Частые синонимы, которыми модель отвечает вместо словарного значения. Нестрогие
# провайдеры (DeepSeek/Gemini) enum соблюдают не всегда, а pydantic на чужом значении
# роняет ВЕСЬ разбор — в досье из-за одного поля терялся готовый психо-портрет целиком.
_SEG_SYNONYMS: dict[str, str] = {
    "образование": "инфобизнес/обучение",
    "обучение": "инфобизнес/обучение",
    "инфобизнес": "инфобизнес/обучение",
    "коучинг": "инфобизнес/обучение",
    "психология": "инфобизнес/обучение",
    "it": "IT/разработка",
    "айти": "IT/разработка",
    "разработка": "IT/разработка",
    "маркетинг": "маркетинг/реклама",
    "реклама": "маркетинг/реклама",
    "финансы": "финансы/инвестиции",
    "инвестиции": "финансы/инвестиции",
    "медицина": "медицина/здоровье",
    "здоровье": "медицина/здоровье",
    "красота": "красота/бьюти",
    "бьюти": "красота/бьюти",
    "строительство": "строительство/ремонт",
    "ремонт": "строительство/ремонт",
    "торговля": "торговля/e-commerce",
    "e-commerce": "торговля/e-commerce",
    "ecommerce": "торговля/e-commerce",
    "услуги": "услуги/сервис",
    "сервис": "услуги/сервис",
    "hr": "HR/подбор",
    "подбор": "HR/подбор",
    "туризм": "туризм/гостеприимство",
    "гостеприимство": "туризм/гостеприимство",
    "транспорт": "транспорт/логистика",
    "логистика": "транспорт/логистика",
}


def normalize_segment(value: str | None) -> str:
    """Ответ модели → значение из SEGMENTS. Неизвестное — «другое», но НЕ исключение:
    потерять весь профиль из-за одного несловарного поля дороже, чем огрубить сегмент."""
    v = (value or "").strip()
    if v in SEGMENTS:
        return v
    low = v.lower()
    if low in _SEG_SYNONYMS:
        return _SEG_SYNONYMS[low]
    for seg in SEGMENTS:                      # «IT» → «IT/разработка» по префиксу
        if seg.lower().startswith(low) and low:
            return seg
    return guess_by_rules(v) or "другое"      # последний шанс — словарь маркеров
