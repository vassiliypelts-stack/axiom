...
_EXTRA_CONTACT_COLS = {
    ...
    "tg_chat_role": "TEXT",
    # Скоринг парсинга по правилам (ТЗ «расширение парсинга»): 1=owner/admin, 2=active
    # в 2+ спарсенных чатах, 3=active в 1 чате, 4=silent (holding — состоит, но молчит,
    # в дальнейшую обработку не передаётся сразу). НЕ путать с contacts.score — то поле
    # занято под AI-скоринг релевантности (agent/enrich_person.py), это простое правило
    # без ML, отдельное по смыслу и по источнику. 1 — высший приоритет (меньше число =
    # важнее), как в самом ТЗ.
    "parse_priority": "INTEGER",
    ...
    # РЕАЛЬНОЕ имя в Telegram (first+last из профиля) и вердикт сверки с тем, что
    # пришло из импорта. Источник ников — выгрузка со стороннего сайта, и там имя с
    # ником разъезжаются: «Дмитрий Норка» с ником @Anna_Dobrokhodskaya. Отправка
    # резолвит по НИКУ (см. telegram._resolve_entity), поэтому письмо уходит другому
    # человеку с обращением «Дмитрий, добрый день!» — и это не видно ни в одном отчёте.
    "tg_name": "TEXT",          # как человек реально подписан в Telegram
    "name_match": "TEXT",       # ok | mismatch | unknown — совпало ли с name из импорта
    "name_checked_at": "TEXT",  # когда сверяли
    ...
    # Горячий лид: согласился на контакт с представителем (см. agent.Reply.hot).
    # NOT NULL = ждём, что владелец напишет ему сам; если за HOT_LEAD_RECHECK_HOURS
    # (см. web/app.py) он так и не написал — бот напоминает человеку проверить личку,
    # дублирует презентацию, шлёт владельцу аларм и чистит поле.
    "hot_since": "TEXT",
    # КОГДА человек согласился (agent.Reply.hot) — в отличие от hot_since НЕ стирается.
    # hot_since — это «ждём владельца прямо сейчас», его снимает _hot_lead_scheduler
    # через HOT_LEAD_RECHECK_HOURS, и после этого факт согласия из базы пропадал: в
    # отчёте кампании лида было не видно вообще, а «кто согласился» приходилось искать
...
_EXTRA_COMPANY_COLS = {
    # Сигналы найма с hh.ru (channels/hh_signals.py). В отличие от реквизитов ЕГРЮЛ,
    # это ПОВОД написать: открытая вакансия = есть деньги и есть боль, которую можно
    # назвать в первом сообщении.
    "checko_checked_at": "TEXT", # когда обогащали через Checko (пусто = ни разу).
                                 # Нужна, чтобы повторный заход не жёг суточный лимит 100/сут
    "hh_employer_id": "TEXT",    # id работодателя на hh (чтобы не искать заново)
    "hh_vacancies": "INTEGER",   # сколько вакансий открыто на момент проверки
    "hh_titles": "TEXT",         # что именно ищут («Менеджер по продажам; Риелтор»)
    "hh_url": "TEXT",            # страница компании на hh
    "hh_checked_at": "TEXT",     # когда проверяли (пусто = ни разу)
...
_EXTRA_CAMPAIGN_COLS = {
    ...
    # ГОЛОСОВЫЕ ЗАГОТОВКИ. voice_after_reply — на каком по счёту ОТВЕТЕ человека
    # уходит голосовое: 1 — сразу после первой его реплики, 2 — после второй и т.д.
    # 0/NULL — голосовые в этой кампании выключены. Холодным первым касанием
    # голосовое не уходит никогда: незнакомцу это агрессивно, а номеру стоит
    # PeerFlood — поэтому минимум единица, то есть «человек уже заговорил с нами».
    "voice_after_reply": "INTEGER DEFAULT 0",
    # Слать голосовое только тем, кто ответил по-человечески заинтересованно, а не
    # отмахнулся. Решает агент по смыслу ответа (intent), см. telegram.py.
    "voice_only_interested": "INTEGER DEFAULT 1",
    ...
    # --- автозапуск кампании по расписанию (web/app.py _campaign_scheduler) ---
    # Статус 'running' сам по себе НИЧЕГО не запускал: он лишь разрешал жать «▶ Запустить»,
    # и каждый заход был ручным нажатием. Прогрев, прокси, пробив и парсинг давно ходят
    # по таймеру сами — рассылка оставалась единственным, что требовало человека, и
    # кампания месяцами шла со скоростью «сколько раз вспомнили нажать» (по 9406 — 22
    # отправки за месяц при очереди 119).
    "auto_send": "INTEGER DEFAULT 0",        # 1 — сервер сам гоняет заходы
    # СКОЛЬКО НОВЫХ КОНТАКТОВ В ДЕНЬ — единственное число, которым задаётся темп.
    # Раньше объём складывался из трёх параметров (auto_interval_min × auto_batch,
    # сверху daily_limit), и ни один не означал «за день»: daily_limit, вопреки
    # названию, резал КАЖДЫЙ заход. При «каждый час по 3» выходило до 45 сообщений в
    # сутки при написанном лимите 3 — оператор задавал одно, получал другое.
    # Планировщик сам считает отправленное за сутки и растягивает остаток по рабочим
    # часам; интервал и размер захода больше не задаются руками.
    "auto_daily": "INTEGER",                 # сколько новых контактов в сутки (NULL = старая логика)
    "auto_interval_min": "INTEGER DEFAULT 60",  # УСТАРЕЛО: осталось для кампаний без auto_daily
    "auto_batch": "INTEGER DEFAULT 3",       # УСТАРЕЛО: там же
    "auto_last_run_ts": "REAL",              # когда последний автозаход (антидубль)
    ...
    # Не используется: тихий номер вместо своего текста стучится как прогреваемый
    # (campaign_send, knock). Колонка осталась от первой версии 25.09.2026.
...
_EXTRA_ACCOUNT_COLS = {
    # Автоматическое замедление при PeerFlood (channels/campaign_send.py). До этой
    # даты холодную рассылку с аккаунта не шлём — сам PeerFlood уже выводил его из
    # ТЕКУЩЕГО захода, но на завтра daily_limit возвращался прежним и аккаунт мог
    ...
    # из текущего захода и всё: через минуту пульт снова показывал его «готов слать», а
    # тест с него молча не доходил. Теперь дата живёт в базе, её видно в выборе
    # отправителя и по ней же _team() не берёт номер в заход. NULL/прошлое = запрета нет.
    "flood_wait_until": "TEXT",
    # Что этот аккаунт делает в слушателе: 'dialogs' — принимает ответы клиентов
    # (кампании, «Диалоги»), 'hits' — сидит в чатах каталога и ловит ключи
    # («Находки»), NULL/'both' — и то, и другое (поведение по умолчанию, как было).
    #
    # ЗАЧЕМ РАЗДЕЛЯТЬ. Два ПРОЦЕССА слушателя на одну сессию завести нельзя: два
    # подключения одним ключом = AuthKeyDuplicated = сожжённый навсегда аккаунт (так
    # потеряли 8 купленных 13.08.2026). А вот развести РОЛИ по РАЗНЫМ аккаунтам можно
    # и полезно: у каждой сессии по-прежнему ровно одно подключение, зато номер,
    # который ведёт живую переписку, перестаёт заодно вычитывать три тысячи чатов —
    # для Telegram это разные профили поведения, и мешать их в одном номере рискованно.
    "listen_role": "TEXT",
    ...
    # НАЗНАЧЕНИЕ аккаунта — чем он занят, в отличие от kind («откуда взялся»).
    #   own     — родной личный номер: автоматика его не трогает вообще;
    #   service — служебный: уведомления владельцу, отчёты, пробив. В рассылку и
    #             прогрев не идёт, поэтому не сгорает вместе с боевыми;
    #   combat  — боевой: холодная рассылка, парсинг, прогрев.
    # Зачем отдельно от protected: protected отвечает на «можно ли трогать», а роль —
    # на «для чего держим». Служебный не «родной» (это купленный номер), но и в
    # рассылку его пускать нельзя: сгорит отправитель уведомлений, и владелец
    # перестанет узнавать о встречах — так 24.08 пропущен живой созвон.
    "acc_role": "TEXT",                   # own | service | combat (пусто = определяем по protected)
...
_EXTRA_CAMPAIGN_CONTACT_COLS = {
    "account_id": "INTEGER",
    # Тихий стук (channels/warmup._knock): когда прогреваемый номер отправил этому
    # контакту короткое «Здравствуйте, Максим?». Непустое значение при пустом
    # sent_at означает «постучались, ответа ещё нет» — по нему слушатель понимает,
    # что откликнувшемуся пора отправить питч, а рассылка — что своё первое
    # сообщение этому человеку писать не нужно, он уже в работе.
    "knock_at": "TEXT",
}
...
_EXTRA_CHAT_COLS = {
    ...
    # по явному запуску оператора/расписанию и на свою пару полей, чтобы прогресс
    # одного процесса не переставлял «докуда дошли» у другого.
    "parse_last_msg_id": "INTEGER",  # до какого msg_id уже досканировали (min_id для дельты)
    "parse_last_at": "TEXT",         # когда последний раз реально спарсили этот чат
}
