...
FOLLOWUP_STEPS = [
    # Шаг 1 — тем же днём, лёгкое «вы на связи?», без давления и без оффера.
    {
        ...
        "variants": [
            ...
            "{name}, тук-тук) видели сообщение?",
            "{name}, вы тут?) а то вдруг сообщение затерялось",
        ],
        "spec_variants": [
            "{name}, вы на связи?) вы ведь всё так же по теме «{spec}»?",
            "{name}, подскажите — вы всё так же занимаетесь темой «{spec}»?",
            "{name}, вы всё ещё работаете в сфере «{spec}»?",
            "{name}, не уверен, что дошло) вы всё так же в теме «{spec}»?",
        ],
    },
    # Шаг 2 — мягкая догадка про занятость + первый намёк на смысл разговора.
    {
        ...
        "variants": [
            ...
            "{name}, видимо, завал?) я коротко: думаю, нам есть смысл объединить усилия",
            "{name}, похоже, сейчас не до меня)) тогда просто оставлю мысль — хочу предложить сотрудничество",
        ],
        "spec_variants": [
            ...
            "{name}, понимаю, что заняты)) но идея простая: вы сильны в «{spec}», я в другом — вместе клиенту можно дать заметно больше",
            "{name}, тишина — значит, работы хватает?) тем интереснее: по «{spec}» у вас поток, и его реально усилить, если объединиться",
        ],
    },
    ...
    # честный выход снимает раздражение и заметно чаще получает ответ «да, интересно».
    {
        ...
        "variants": [
            ...
            "{name}, закрываю вопрос с моей стороны) станет актуально — пишите в любой момент",
            "{name}, больше не пишу, чтобы не надоедать)) если что — я на связи и всегда рад обсудить",
        ],
        "spec_variants": [
            ...
            "{name}, закрываю тему с моей стороны) станет интересно партнёрство по «{spec}» — пишите в любой момент",
            "{name}, не хочу быть навязчивым)) если по «{spec}» появится интерес к сотрудничеству — я на связи",
        ],
    },
]
...
def _followup_text(step: dict, contact_row, streak: int) -> str:
    ...
    # Сдвиг на streak, а не голый contact_id: иначе человек с id%4==0 получал бы
    # «нулевой» вариант на КАЖДОМ шаге — то есть всю лесенку в одной интонации, а
...
def collect_due(conn, now: datetime | None = None) -> list[Action]:
    ...
    for c in contacts:
        ...
        if queued:
            continue
        # ДИАЛОГ УЖЕ ЗАКРЫТ — НЕ ДОЖИМАЕМ.
        #
        # hot_since ставит агент в момент закрытия («спасибо за интерес, с вами
        # свяжется представитель»), а дальше по промпту положена ТИШИНА: продавать
        # больше нечего, человека ведёт живой представитель. Планировщик про это не
        # знал и видел только «последнее сообщение наше, ответа нет» — 23.09 тестовый
        # контакт #295, закрытый ещё 19.09, получил дожим «добавлю главное, о чём
        # обычно спрашивают» через четыре дня после «с вами свяжется представитель».
        # Для получателя это бот, забывший, о чём договорились.
        if c["hot_since"] if "hot_since" in c.keys() else None:
            continue
        # Встреча назначена — тем более молчим: дальше работает напоминание о
        # созвоне (выше), а не дожим молчуна.
        ...
        #
        # Раньше свой шаблон кампании прикладывался ПОСЛЕ FOLLOWUP_STEPS, и человек
        # сначала получал общие шаги — а они написаны под нетворкинг и прямо предлагают
        # «объединить усилия» и «партнёрство». В продающей кампании это выглядит как
        # сообщение из другой вселенной: 17-18.09 по 9407 трое получили после двух
        # писем про участки в Крыму текст «по "Консалтинг, коучинг; PRO" у вас поток,
        # и его реально усилить, если объединиться» ({spec} подставился из HR-базы,
        # откуда часть контактов и пришла). Для получателя это бот-неадекват.
        #
