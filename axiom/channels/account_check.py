...
async def check_one(cand: Candidate) -> Result:
    ...
    # ещё нет в БД и слушатель их не держит. Не прогоняй по папке с аккаунтами, которые
    # УЖЕ заведены и живут в пульте: проба «напрямую» идёт с IP этой машины, а слушатель
    # на сервере держит ту же сессию через прокси — Telegram увидит один ключ с двух
    # адресов и сожжёт его навсегда (так потеряно 8 лотов, см. шапку session_check.py).
