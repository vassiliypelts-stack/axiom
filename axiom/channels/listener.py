...
def _should_reply(acc_id: int, contact_id: int | None = None) -> bool:
    ...
    ГЛАВНОЕ: агент общается только с теми, кому кампания ФАКТИЧЕСКИ писала с этого
    аккаунта (строка в campaign_contacts). Все прочие — не его собеседники, даже если
    их карточка есть в книжке: туда людей кладут парсинг чатов, импорт и обогащение,
    безо всякой рассылки. Написал такой человек сам — входящее ложится в «Диалоги»,
    и отвечает оператор, а не бот.

    Раньше правилом было «аккаунт active → отвечаем всем подряд», и этого хватало,
    чтобы бот заговорил с кем угодно из базы. Ступень прогрева при этом значения не
    имеет: рассылка отбирает команду по `status <> 'banned'` (campaign_send._team),
    то есть пишет и с прогреваемых аккаунтов, и человек, ответивший на такое письмо,
    обязан получить ответ — по факту записи в campaign_contacts, а не по статусу.
    ...
    когда-то попал в таблицу контактов. Разрешаем ответ ТОЛЬКО пока аккаунт СЕЙЧАС
    состоит в команде той же кампании, что написала этому человеку — не «когда-то
    писали», см. проверку через campaign_accounts ниже.
    ...
    with database.get_conn() as conn:
        ...
        if row["status"] == "banned" or not contact_id:
            return False
        # ГЛАВНОЕ ПРАВИЛО: агент разговаривает ТОЛЬКО с теми, кому кампания реально
        # писала с ЭТОГО аккаунта. Раньше «активный» аккаунт отвечал кому угодно, чей
        # contact_id нашёлся в книжке, — а книжка полна людей, которых туда положил
        # парсинг чатов, импорт и обогащение, безо всякой рассылки. Любой такой человек,
        # написав нашему номеру сам, получал ответ бота: он этого не просил, и мы ему
        # ничего не отправляли. Проверяем по campaign_contacts — это список тех, кому
        # рассылка фактически ушла (строка появляется в момент отправки, см.
        # campaign_send и warmup._ca_mix). Нет строки — молчим, входящее просто
        # ложится в «Диалоги», и оператор решает сам.
        #
        # Побочно закрывается взаимный прогрев: аккаунты греются перепиской между собой,
        # но друг друга в campaign_contacts нет, поэтому агент в эту переписку не лезет.
        in_list = conn.execute(
            "SELECT 1 FROM campaign_contacts WHERE contact_id=? AND account_id=? LIMIT 1",
            (contact_id, acc_id)).fetchone()
        if not in_list:
            # ЗАПАСНОЙ ПРИЗНАК — ФАКТ ПЕРЕПИСКИ. Строка в campaign_contacts не вечная:
            # «🔄 Обнулить тест» удаляет её намеренно (см. app.campaign_test_reset), а
            # рассылка потом дописывает опенер с ДРУГОГО аккаунта команды. В зазоре
            # получалось так: человеку написали и он ответил, но пары (контакт, этот
            # аккаунт) в списке уже/ещё нет — агент молчал, хотя диалог живой.
            # 10.09.2026 на живом тесте так утонули оба своих номера (913 и 988):
            # опенеры уходили, ответы «что предлагаете?» / «и???» ложились в
            # «Диалоги», и ни на один агент не ответил.
            #
            # Само правило не ослабляем: мы по-прежнему разговариваем ТОЛЬКО с теми,
            # кому МЫ написали С ЭТОГО аккаунта — просто спрашиваем об этом не список
            # рассылки, а сами сообщения. Взаимный прогрев сюда не попадает: наши
            # аккаунты пишут друг другу напрямую, минуя messages контакта.
            wrote = conn.execute(
                "SELECT 1 FROM messages WHERE contact_id=? AND account_id=? "
                "AND direction='out' LIMIT 1", (contact_id, acc_id)).fetchone()
            if not wrote:
                return False
        if not row["protected"]:
            return True
        # РОДНОЙ аккаунт: сверх общего правила — разрешаем ответ, только если он ПРЯМО СЕЙЧАС состоит в
        # команде кампании, которая переписывается с этим контактом — не просто
        # «когда-то написал». Раньше проверялось голым фактом из messages без
        # срока и без привязки к текущей команде: 14.08.2026 на живом прогоне
        # родной номер хозяина (protected, снят с команды ещё в августе) продолжал
        # как ни в чём не бывало отвечать в двух старых диалогах, начатых через
        # ЦА-микс прогрева ДО того, как аккаунт пометили родным — агент довёл один
        # разговор до «назначена встреча» с живым человеком, которого хозяин лично
        # знает. Снятие protected/удаление из команды не останавливало это НИКОГДА:
        # разрешение выдавалось раз и навсегда первой строкой в messages. Теперь
        # разрешение живёт ровно столько, сколько аккаунт числится в команде той
        # же кампании, что и переписка — убрали из команды, разговор глохнет сам,
        # человек продолжает видеть только оператора-хозяина, а не бота.
        started = conn.execute(
            "SELECT 1 FROM campaign_contacts cc "
            "JOIN campaign_accounts ca ON ca.campaign_id=cc.campaign_id AND ca.account_id=cc.account_id "
            "WHERE cc.contact_id=? AND cc.account_id=? LIMIT 1",
            (contact_id, acc_id)).fetchone()
    return bool(started)
...
async def _connect(acc: dict):
    # БЕЗ СВОЕГО ПРОКСИ НЕ ПОДКЛЮЧАЕМ. build_client при пустом proxy молча падает на
    # прокси главного .env-аккаунта — и дальше два зла разом:
    #   1) сессия уходит в Telegram с ЧУЖОГО IP. Стоит любому другому модулю (или
    #      самому оператору) поднять её штатным путём, Telegram видит один ключ с двух
    #      адресов и жжёт его НАВСЕГДА («authorization key was used under two different
    #      IP addresses simultaneously»). Так 13.08.2026 разом сгорели 8 купленных
    #      аккаунтов — ровно те, у кого поле proxy было пустым;
    #   2) несколько таких аккаунтов светят Telegram ОДИН И ТОТ ЖЕ .env-IP — почерк
    #      фермы, бан пачкой.
    # Тот же гейт давно стоит у прогрева (database.warming_accounts), а слушатель его
    # не имел: он берёт аккаунты только по наличию сессии и статусу (_listenable).
    # Пропуск здесь безопаснее подключения: аккаунт остаётся цел, а причина видна в
    # пульте — прокси раздаётся кнопкой «🆓 Бесплатный прокси» в «Аккаунтах».
    if not (acc.get("proxy") or "").strip():
    ...
    while True:
        ...
        async def _try(a: dict) -> None:
            try:
                ...
                #
                # НО: если сессия только что признана мёртвой (см. блок выше —
                # AuthKeyDuplicated / revoked / «не авторизована»), лечить нечего.
                # Прокси тут ни при чём, а heal() ради проверки САМ поднимает Telethon
                # этим же ключом. Аккаунт не подключается → следующий тик через 30 сек
                # снова зовёт heal → снова подключение мёртвым ключом. В логе 10.09 это
                # дало 88 заходов «проверяю 1 аккаунт» за 4 минуты по кругу, и каждый —
                # ещё один выход сожжённой сессии в эфир. Мёртвую сессию чинит только
                # повторный вход из «Аккаунтов», а не смена IP.
                if unauth or name in ("AuthKeyDuplicatedError", "AuthKeyUnregisteredError",
                                      "SessionRevokedError", "SessionExpiredError",
                                      "AuthKeyInvalidError"):
                    _log(f"[#{a['id']}] сессия мертва ({name}) — прокси не трогаю, "
                         f"нужен повторный вход в разделе «Аккаунты»")
                    return
                try:
