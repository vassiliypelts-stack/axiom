...
-- База возражений кампании: что люди отвечают и как это отрабатывать.
--
-- Копилка, которая растёт от живой переписки. Держать отработки в промпте не
-- выходит: он пухнет с каждой правкой, а новое возражение нельзя добавить, не
-- переписав весь текст. Здесь каждое возражение — отдельная строка, которую
-- владелец правит, не трогая сценарий.
--
-- Наполняется с двух сторон. Вручную — владелец вбивает то, что слышал сам.
-- Автоматически — агент складывает сюда реплики, которые разметил как
-- objection (agent/agent.py, Reply.intent): возражение попадает в базу с
-- пустым answer и ждёт, пока владелец напишет ответ. Пока answer пуст, в
-- промпт эта строка не идёт — агент не должен импровизировать там, где
-- владелец ещё не решил, что отвечать.
CREATE TABLE IF NOT EXISTS campaign_objections (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    campaign_id INTEGER NOT NULL,
    title       TEXT,        -- короткое имя: «Дорого», «Далеко», «Не верю»
    objection   TEXT,        -- как это звучит от человека (дословно или обобщённо)
    answer      TEXT,        -- как отрабатывать; пусто = ещё не решено, в промпт не идёт
    enabled     INTEGER DEFAULT 1,
    hits        INTEGER DEFAULT 0,   -- сколько раз встретилось в диалогах
    source      TEXT,        -- 'manual' | 'auto' (кто завёл строку)
    contact_id  INTEGER,     -- у авто-строки: в каком диалоге впервые встретилось
    last_seen   TEXT,
    created_at  TEXT DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_objections_campaign ON campaign_objections(campaign_id);
