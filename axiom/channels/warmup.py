...
#   • СВОЙ ЧАС У КАЖДОГО НОМЕРА (_upkeep_due_now). Прогон идёт часто, но номер
#     выходит на связь в свой час дня и примерно два дня из трёх. Без этого
#     23.09 в 22-23 часа ожили 34 аккаунта подряд, а в 4-5 утра — ещё 11:
#     десятки «личных» переписок в одну минуту и ночью выдают ферму вернее,
#     чем любой текст. Ночью (вне UPKEEP_HOURS) прогон не идёт вовсе.
#   • ОЧЕРЕДЬ по давности (last_upkeep_at) и доля UPKEEP_SHARE=1.0: кандидатами
#     считаем всех боевых, а кто реально пишет — решает личный час. Доля 0.25
#     давала номеру живость раз в четыре дня, и 22.09 по 9407 восемь
#     отправителей слегли в PeerFlood за три часа: холодные исходящие есть,
#     ничего другого нет.
...
UPKEEP_DIALOGS = [
    ...
    # Дюжины сценариев мало: на 22 номерах пары повторяются за пару дней, и
    # одинаковые реплики в разных чатах — такой же почерк, как одинаковое время.
    ("о, привет) как жизнь", "да ничего, всё по-старому) сам как"),
    ("ты сегодня занят вечером?", "пока не знаю, а что хотел"),
    ("как настроение?", "рабочее) пятницу жду"),
    ("давно тебя не слышно", "да закрутился, сам хотел написать"),
    ("ты где сейчас, в городе?", "ага, никуда не уезжал"),
    ("как сам, не болеешь?", "тьфу-тьфу) нормально всё"),
    ("слушай, а ты спортом занимаешься?", "так, по мелочи) зал иногда"),
    ("что делаешь?", "да так, дела разгребаю"),
    ("привет) не отвлекаю?", "не, нормально) говори"),
    ("как прошли выходные?", "тихо) отоспался наконец"),
    ("у тебя всё нормально?", "да, всё хорошо) а что"),
    ("читал что-нибудь интересное?", "да руки не доходят честно говоря"),
    ("как работа идёт?", "потихоньку, не жалуюсь"),
    ("ты обедал уже?", "только собираюсь) а ты"),
    ("привет, как дела на личном фронте)", "та всё стабильно) не жалуюсь"),
    ("холодает уже, заметил?", "ага, куртку достал вчера"),
    ("ты машину так и не поменял?", "не) пока езжу на старой"),
    ("что нового у тебя?", "да ничего особенного, всё как обычно"),
]
...
def _msk_now():
    """Текущее время по Москве — в нём живут получатели и владелец."""
    import datetime as _dt
    try:
        from zoneinfo import ZoneInfo
        return _dt.datetime.now(ZoneInfo("Europe/Moscow"))
    except Exception:  # noqa: BLE001 — нет tzdata: сервер в UTC, МСК = +3
        return _dt.datetime.utcnow() + _dt.timedelta(hours=3)


# Окно, в котором люди переписываются. Ночью прогон не идёт вовсе.
UPKEEP_HOURS = (9, 23)


def _upkeep_daytime() -> bool:
    return UPKEEP_HOURS[0] <= _msk_now().hour < UPKEEP_HOURS[1]


def _upkeep_due_now(acc: dict) -> bool:
    """Пришёл ли «личный час» этого номера.

    Раньше прогон брал всю команду разом: 23.09 в 22-23 часа ожили 34 номера
    подряд. Даже с разными сценариями это один почерк — десятки аккаунтов
    начинают переписку в одну и ту же минуту.

    Каждому номеру считаем СВОЙ час активности внутри дневного окна: хеш от id и
    даты, поэтому в пределах суток он стабилен (перезапуск не превращает график в
    лотерею), а изо дня в день сдвигается. Плюс номер выходит на связь не каждый
    день: примерно два дня из трёх, как обычный человек.
    """
    import hashlib
    now = _msk_now()
    seed = hashlib.sha256(f"{acc['id']}:{now.date().isoformat()}".encode()).digest()
    lo, hi = UPKEEP_HOURS
    my_hour = lo + seed[0] % (hi - lo)
    if seed[1] % 3 == 0:
        return False                      # сегодня этот номер молчит
    return now.hour >= my_hour            # его час настал (и позже — догоняем)
...
async def run_upkeep(only_id: int | None = None) -> None:
    ...
    """
    database.init_db()
    # НОЧЬЮ НЕ ПИШЕМ. Прогон шёл каждые 6 часов по часам сервера, и 23.09 в 4-5 утра
    # по Москве одиннадцать номеров «переписывались» между собой. Живой человек в это
