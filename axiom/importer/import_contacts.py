...
ALIASES = {
    ...
    # «Наименование» — так называется колонка с ИМЕНЕМ ЧЕЛОВЕКА в выгрузках каталогов
    # экспертов (hrtime, vsetreningi): там каждая строка — человек, а не юрлицо.
    # Без этой строки файл на 100 экспертов давал «импортировано 0»: имя не
    # распозналось, телефона в таких выгрузках нет, и брать из строки было нечего.
    # На импорт КОМПАНИЙ это не влияет — там своя карта колонок (_COL_MAP в web/app.py).
    "наименование": "name", "фио руководителя": "name", "руководитель": "name",
    "полное имя": "name", "ф.и.о.": "name", "фамилия имя отчество": "name",
    ...
    # КОРОТКАЯ тема → specialization: она подставляется в шаблон одной строкой
    # («вы всё так же работаете как {спец}?»), поэтому туда годится «увеличение продаж»,
    # но не простыня.
    "specialization": "specialization", "специализация": "specialization",
    "категории": "specialization", "категория": "specialization",
    "чем занимается": "specialization", "деятельность": "specialization",
    "направление": "specialization", "сфера": "specialization",
    "услуги": "specialization", "тема": "specialization",
    # ДЛИННЫЙ текст → niche: в выгрузках каталогов «Описание» это 400-1000 знаков.
    # Раньше он тоже шёл в specialization и превращал первое касание в простыню.
    # Агент читает niche как контекст, но в реплику целиком не тащит.
    "описание": "niche", "description": "niche", "о себе": "niche",
    "about": "niche", "bio": "niche", "био": "niche",
    "навыки": "niche", "основные направления": "niche",
    ...
    # Ссылка на первоисточник (профиль в каталоге, сайт, соцсеть). Раньше такая колонка
    # не распознавалась и уезжала в «Заметки» — а это единственная ниточка к человеку,
    # когда телефона в выгрузке нет: по ней его дообогащают.
    "ссылка": "site", "профиль": "site", "url": "site", "сайт": "site",
    "link": "site", "site": "site", "страница": "site", "ссылка на профиль": "site",
}
