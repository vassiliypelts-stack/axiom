...
def _roll_future(dt: datetime, now: datetime) -> datetime:
    """Встречу не назначаем в прошлое — но поправка разная в зависимости от того,
    НАСКОЛЬКО дата в прошлом.

    Найдено вживую: «давайте сегодня в 15:00», обработано в 15:03 — dt на три минуты
    в прошлом, и старая версия прыгала на ГОД вперёд (2026 → 2027), а не на завтра.
    Причина в путанице двух разных ситуаций под одной проверкой:
      • dt недалеко в прошлом (задержка обработки, ночная пауза, «сегодня в X» с X,
        который только что миновал) — это НЕ «имел в виду прошлый год». Правка —
        на СЛЕДУЮЩИЙ ДЕНЬ. Порог 3 суток с запасом покрывает даже отложенный ответ
        (ночью агент молчит до утра — см. antiban.within_work_hours).
      • dt далеко в прошлом (обычно недели-месяцы) — типичный случай «12 августа»
        без года, когда 12 августа в этом году уже было. Тут годовой сдвиг оправдан.
    """
    if dt >= now:
        return dt
    if (now - dt) < timedelta(days=3):
        # докидываем СУТКАМИ (не одним прыжком), пока не окажемся в будущем — час/минута
        # сохраняются, меняется только число. Одного +1 дня не хватило бы при задержке
        # больше суток (например, ночная пауза): результат остался бы в прошлом.
        while dt < now:
            dt += timedelta(days=1)
        return dt
    try:
