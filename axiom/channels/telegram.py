...
# Вводные обороты, после которых ОБЯЗАНА идти суть: «тогда по сути», «сейчас
# расскажу», «если коротко». Сами по себе они не несут ничего — человек ждёт
# продолжения, а его нет.
_DANGLING_RE = re.compile(
    r"^(отлично|хорошо|супер|понял[аи]?|принял[аи]?|ясно|здорово|прекрасно)?[\s,.!—-]*"
    r"(тогда|сейчас|давайте|итак)?[\s,.!—-]*"
    r"(по сути|по делу|расскажу|объясню|коротко|в двух словах|поясню|напишу)"
    r"[\s.!:—-]*$",
    re.I,
)


def _is_dangling(parts: list[str]) -> bool:
    """Ответ модели — вводная фраза без самой сути? Такой не отправляем.

    Ловим узкий случай: ОДНА короткая реплика, которая обещает продолжение и на этом
    заканчивается («Отлично, тогда по сути.»). Всё остальное пропускаем: короткий
    ответ сам по себе нормален («Да, конечно», «Понял, спасибо»), и глушить живой
    диалог из-за краткости нельзя. Признак именно обрыва — обещание рассказать,
    после которого ничего не сказано, и отсутствие вопроса собеседнику."""
    live = [p.strip() for p in (parts or []) if p and p.strip()]
    if len(live) != 1:
        return False                      # 2-3 реплики — это уже содержательный ответ
    one = live[0]
    if len(one) > 90 or "?" in one:       # длинная реплика или вопрос — суть есть
        return False
    return bool(_DANGLING_RE.match(one))
...
async def _agent_reply(event, contact_id: int, username: str | None,
    ...
    try:
        ...
        # МОЛЧИМ. Раньше здесь уходила «человеческая» отговорка (_STALL_REPLIES:
        # «сорри, отвлекся», «звонок словил») — чтобы человек не остался без единого
        # слова. На практике это оказалось хуже молчания, сразу по двум причинам.
        #
        # 1. Отговорка не отвечает на вопрос. Все варианты сводились к «повтори, что
        #    писал?» — хотя написанное прекрасно видно в истории. На прямое «а Вы кто
        #    и кого ищете?» это читается как уход от ответа, а не как занятость.
        # 2. Отговорка ЛОМАЛА догон. catchup_reply_pass берёт диалоги, где последнее
        #    сообщение — входящее. Отправив отговорку, мы делали последним исходящее,
        #    и диалог для догона исчезал: человек оставался с «секунду, отвечу»
        #    навсегда. Вживую (28.08.2026) так и вышло — ответ ушёл через 10 часов
        #    после вопроса, «секунду» посреди следующего утра.
        #
        # Теперь: не пишем ничего, диалог остаётся с входящим в хвосте — и как только
        # агент починится, ближайший тик догона ответит ПО СЦЕНАРИЮ, без упоминания
        # паузы. Оператор о падении знает из колокольчика (_notify_agent_down выше).
        return
    ...
    # ОБРУБОК. Модель иногда возвращает только вводную фразу без самого ответа:
    # «Отлично, тогда по сути.» — и всё (11.09.2026, DeepSeek, контакт 295: человек
    # спросил «и что вы предлагаете?», получил эту строку и остался с ней навсегда).
    # Формально ответ валиден — одна непустая строка, схема довольна, повтора нет.
    # А для человека это хуже молчания: собеседник пообещал перейти к делу и пропал.
    #
    # Лучше промолчать: диалог остаётся с входящим в хвосте, и ближайший тик догона
    # (catchup_reply_pass, раз в 10 мин) переспросит модель заново — уже по сценарию.
    if _is_dangling(reply.reply_parts):
        print(f"[{contact_id}] ⚠ обрубок от модели: {reply.reply_parts!r} — не шлю, "
              f"диалог ждёт догона")
        with database.get_conn() as conn:
            database.add_event(
                conn, "agent_error", "✂️ Агент оборвал ответ на полуслове",
                f"Модель вернула только вводную фразу без сути: «{' '.join(reply.reply_parts)[:120]}». "
                f"Не отправлено — человек не должен остаться с обрывком. Догон переспросит "
                f"модель в ближайшие 10 минут. Если повторяется часто — модель слабовата "
                f"для диалога, стоит вернуть Claude в «Аккаунты → модель агента».",
    ...
    with database.get_conn() as conn:
        ...
        # Горячий лид (agent.Reply.hot) — согласился, что с ним свяжется представитель.
        # hot_since — метка «жду владельца»: тик _hot_lead_scheduler (web/app.py) через
...
class _PseudoEvent:
    ...
    Досылка-догон (см. catchup_reply_pass) отвечает на сообщение, пришедшее ночью
...
async def catchup_reply_pass(limit: int = 20) -> int:
    """Ответить всем, кто написал и остался без ответа — по любой причине.

    Два случая, оба про «человек написал, мы промолчали»:
      1. НЕРАБОЧЕЕ ВРЕМЯ. Ночью агент молчит намеренно (см. listener: 09:00-21:30
         МСК) — ответ в три часа ночи выдаёт автоматику вернее всего остального.
      2. АГЕНТ УПАЛ. Модель не собрала ответ (кончились кредиты, не-JSON после всех
         повторов) — см. _agent_reply. Раньше туда уходила отговорка «сорри,
         отвлекся», и она же ЛОМАЛА этот проход: последним сообщением становилось
         исходящее, диалог переставал сюда попадать, человек оставался с «секунду,
         отвечу» навсегда. Теперь при падении мы молчим — и диалог ждёт здесь.

    Берём диалоги, где последнее сообщение входящее, и отвечаем ОБЫЧНЫМ путём агента
    (_agent_reply). Это принципиально: свой текст функция не пишет ни одной буквы,
    а заново зовёт модель с промптом кампании и полной историей — поэтому поздний
    ответ идёт ПО СЦЕНАРИЮ и про паузу не упоминает. Сценарий живёт в кампании, а не
    в памяти упавшего процесса, и от задержки не теряется.

    Зовётся раз в 10 минут в рабочие часы (web/app._night_reply_scheduler), так что
    упавший вечером агент подхватывает диалог сам, как только оживёт.

    Возвращает число отвеченных. Вне рабочих часов не делает ничего.
    """
    # ВНЕ ОКНА ДОГОНЯЕМ ТОЛЬКО СВОИ ТЕСТ-НОМЕРА. Живым людям ночью не пишем — это
    # правило остаётся. Но глушить весь проход было слишком грубо: тестовый прогон
    # оператор гоняет тогда, когда проверяет сценарий, сплошь и рядом поздно вечером,
    # и молчание агента в этот момент читается как поломка — 10.09.2026 ровно так и
    # вышло (ответы в 21:51 МСК, окно закрылось в 21:30). Ниже в _agent_reply то же
    # исключение для is_test уже сделано, и антибан-докстринг его обещает — здесь
    # проход обрывался раньше, чем до него доходило дело.
    only_test = not antiban.within_work_hours()
    with database.get_conn() as conn:
        rows = conn.execute(
            ...
            + ("AND COALESCE(c.is_test,0)=1 " if only_test else "")
...
if __name__ == "__main__":
    ...
    if args.night_replies:
        n = asyncio.run(catchup_reply_pass())
