...
_LINE_RULES: list[tuple[re.Pattern, str, str]] = [
    ...
    # Утверждённый подход — сразу дать понятный оффер. «Можно задать короткий
    # вопрос?» оставляет человека без контекста и просит разрешение на разговор.
    (re.compile(r"^\s*(можно|могу)\s+задать\s+(?:вам\s+)?(?:коротк\w*\s+)?вопрос\s*\?\s*$", re.I),
     "severe", "не спрашиваем разрешения задать вопрос — сразу формулируем оффер"),
