...
async def heal(ids: list[int] | None = None, warming_only: bool = True) -> dict:
    ...
    if not ids:
        # ГЛАВНАЯ ЗАЩИТА: пока слушатель ВКЛЮЧЁН, менять прокси нельзя НИКОМУ.
        #
        # Ниже мы пробуем спросить у слушателя список подключённых (CLIENTS) — но это
        # работает, только если heal живёт В ТОМ ЖЕ процессе, что и слушатель. Плановое
        # автолечение запускается ОТДЕЛЬНЫМ процессом (web/app.py: subprocess.run
        # "--heal --all"), а там CLIENTS — пустой словарь свежего импорта. То есть
        # проверка молча вырождалась в «никто не подключён», и heal каждые 5 минут
        # переставлял прокси прямо под живыми сессиями.
        #
        # 09.09.2026 так сгорело 4 аккаунта за час (Василий236, Василий317, Ольга987,
        # Татьяна283 — все AuthKeyDuplicatedError), причём трое из них были подняты из
        # запасок этим же утром. В логе за сутки НИ РАЗУ не встретилось «пропускаю
        # подключённых» — защита не сработала ни разу с момента написания.
        #
        # Флаг listener_enabled лежит в БД, а не в памяти, поэтому виден из любого
        # процесса. Аккаунт, отвалившийся по-настоящему, всё равно вылечится: слушатель
        # сам зовёт heal(ids=[...]) точечно в момент, когда этот клиент отключён.
        with database.get_conn() as conn:
            listener_on = database.get_setting(conn, "listener_enabled", "on") != "off"
        if listener_on:
            print(f"[heal] слушатель ВКЛЮЧЁН — плановую замену прокси пропускаю целиком "
                  f"({len(accs)} аккаунтов). Смена IP под живой сессией = "
                  f"AuthKeyDuplicatedError и сожжённый ключ.")
            return {"checked": 0, "alive_kept": 0, "healed": 0, "no_pool": 0,
                    "skipped_listener_on": len(accs)}
        try:
            from channels.listener import CLIENTS
            busy = {aid for aid, cl in list(CLIENTS.items())
                    if getattr(cl, "is_connected", None) and cl.is_connected()}
