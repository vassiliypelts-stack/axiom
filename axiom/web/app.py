...
# Цена в названии — не украшение: сценарий кампании уходит в модель НА КАЖДОЕ
# сообщение вместе с историей, и разница между Opus и Haiku это разница в 25 раз на
# том же диалоге. 17.09.2026 тесты на Sonnet съели полдоллара за час, и понять это
# по одному названию модели было нельзя. Порядок — от дешёвых к дорогим.
AGENT_MODEL_CHOICES = [
    {"id": "deepseek:deepseek-chat",
     "name": "DeepSeek — самый дешёвый (~0,1¢/ответ), но в диалоге срывается"},
    {"id": "gemini:gemini-2.5-flash",
     "name": "Gemini 2.5 Flash — дёшево и быстро (~0,1¢/ответ)"},
    {"id": "openai:gpt-4.1-mini",
     "name": "GPT-4.1 mini — дёшево, уверенно держит инструкции (~0,2¢/ответ)"},
    {"id": "claude-haiku-4-5",
     "name": "Claude Haiku 4.5 — дёшево и послушно (~0,4¢/ответ)"},
    {"id": "openai:gpt-4.1",
     "name": "GPT-4.1 — сильный диалог (~1,2¢/ответ)"},
    {"id": "claude-sonnet-5",
     "name": "Claude Sonnet 5 — рабочая лошадка для диалогов (~1,3¢/ответ)"},
    {"id": "claude-opus-5",
     "name": "Claude Opus 5 — самый умный, дорогой (~6¢/ответ)"},
]
    ...
    while True:
        ...
        try:
            ...
            for a in actions:
                ...
                # напоминание/«не дошёл»/дожим уходят ТОЛЬКО тем, кому кампания реально писала
                # ЭТИМ аккаунтом (campaign_contacts). Раньше это правило стояло только на
                # ответах на входящие — а сама эта сторожевая петля продолжала слать
                # напоминания и «не дошёл» кому угодно с deals/активным диалогом, включая
                # личных знакомых, с которыми говорил владелец лично, а не рассылка (см.
                # Роман Галанкин, инцидент 08.2026 — бот прислал «не получилось созвониться»
                # ЛИЧНОМУ знакомому спустя дни после того, как переписка вообще была не из
                # кампании). Deal тут мог создаться просто из-за детекта «договорились»
                # в личном разговоре — рассылки не было ни разу.
                with database.get_conn() as conn:
                    in_campaign = conn.execute(
                        "SELECT 1 FROM campaign_contacts WHERE contact_id=? AND account_id=? LIMIT 1",
                        (a.contact_id, row["account_id"])).fetchone()
                if not in_campaign:
...
def _auto_send_plan(conn, camp) -> tuple[int, str]:
    ...
    # За раз — ОДИН контакт. Было «не больше 3», и заход слал их подряд с паузой
    # 40-130 секунд: для номера, которому незнакомцы почти не отвечают, три холодных
    # письма в пределах пяти минут — уже серия. 22.09 по 9407 все восемь отправителей
    # словили PeerFlood за три часа при лимите 2/сутки на аккаунт и отдельном прокси
    ...
    with database.get_conn() as conn:
        for row in rows:
            ...
            if target == "contacts":
                ...
                # Импорт ВОЗВРАЩАЕТ карточку из корзины. Дедуп находит и удалённые
                # (корзина — это метка deleted_at, а не удаление), и без этой строки
                # повторный импорт молча обновлял их прямо в корзине: отчёт «загружено
                # 100», а в разделе «Контакты» пусто — ровно то, что выглядит как
                # сломанный импорт. Раз человека грузят заново, он нужен активным.
                if cid:
                    conn.execute("UPDATE contacts SET deleted_at=NULL "
                                 "WHERE id=? AND deleted_at IS NOT NULL", (cid,))
                    added += 1
            else:
...
async def import_preview(file: UploadFile = File(...), target: str = Form("contacts")) -> JSONResponse:
    ...
    return JSONResponse({
        ...
        # Предупреждение, а НЕ запрет: карточки без телефона грузятся и живут в базе как
        # сырьё для обогащения. Формулировка это отражает — иначе оператор читает
        # «писать некуда» как «импорт не сработает» и не жмёт кнопку.
        "warn": ("В файле нет ни телефона, ни Telegram. Контакты загрузятся, но писать им "
                 "сразу будет нечем — в рассылку они не попадут, пока не появится номер "
                 "или @ник. Если есть колонка со ссылкой на профиль — укажи её, по ней "
                 "человека можно дообогатить позже."
...
def _camp_row(conn, r) -> dict:
    ...
    # снимает только 'done' (аудитория кончилась) или ручная пауза. Отработал заход —
    # процесс умер, а флаг остался, и кампания месяцами числится «идёт» без единой
    # отправки. Пульт из-за этого врал: «Идёт» пульсировало третьи сутки при нуле.
    # date('now') без 'localtime': sent_at пишется схемой как datetime('now'), то есть
