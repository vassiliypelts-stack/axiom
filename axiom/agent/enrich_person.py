...
SYSTEM = (
    ...
    "ЕСЛИ в контексте есть блок «ЧТО ПРЕДЛАГАЕМ МЫ» — сверь с ним offer и niche человека. "
    "Когда человек занимается тем же самым (продаёт то же, что предлагаем мы), он НЕ покупатель, "
    "а коллега по рынку: пиши rec_message как заход на партнёрство, обмен опытом или взаимный "
    "поток клиентов — и никогда не предлагай ему то, что он сам продаёт. Это видно сразу и "
    "закрывает диалог навсегда. Когда рынок смежный, а не тот же — можно предлагать, "
    "опираясь на его задачи. "
...
def _our_offer() -> str:
    """Чем занимаемся МЫ — из активного проекта (цель + идеальный результат).

    Без этого портрет пишется в вакууме: человеку, который сам внедряет ИИ, шёл заход
    «мы помогаем внедрять ИИ». Модель не может распознать коллегу, если не знает, с чем
    сравнивать. Берём последний неархивный проект с заполненной целью — это и есть
    «чем мы сейчас занимаемся»."""
    try:
        with database.get_conn() as conn:
            row = conn.execute(
                "SELECT name, goal, ideal_result FROM projects "
                "WHERE IFNULL(status,'')<>'archived' AND IFNULL(goal,'')<>'' "
                "ORDER BY id DESC LIMIT 1").fetchone()
    except Exception:  # noqa: BLE001 — портрет важнее, чем контекст оффера
        return ""
    if not row:
        return ""
    parts = [f"проект «{row['name']}»" if row["name"] else "", row["goal"] or ""]
    if row["ideal_result"]:
        parts.append(f"желаемый результат: {row['ideal_result']}")
...
def _build_context(contact: dict, posts: list) -> str:
    ...
    offer = _our_offer()
    if offer:
        lines += ["", f"ЧТО ПРЕДЛАГАЕМ МЫ: {offer}"]
    lines += ["", f"Его сообщения из чатов ({len(posts)} шт., свежие сверху):"]
    for p in posts:
