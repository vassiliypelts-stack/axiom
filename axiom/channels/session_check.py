...
ОСТОРОЖНО С ЭТИМ ФОЛБЭКОМ (см. _check_one). Telegram жжёт ключ не за параллельные
подключения как таковые, а за подключения С РАЗНЫХ IP: текст ошибки —
«authorization key was used under two different IP addresses simultaneously». Проба
«напрямую» идёт с IP САМОГО СЕРВЕРА, мимо прокси аккаунта. Пока слушатель держит эту
же сессию через прокси (а он держит все живые аккаунты), такая проба даёт Telegram
ровно картину угона — и аккаунт сгорает навсегда. Проверка живости ходит по всем
аккаунтам разом, поэтому промах убивает их пачкой: так лежит 8 купленных лотов.
Поэтому напрямую пробуем ТОЛЬКО когда слушатель выключен и держать сессию некому.
Выигрыш от фолбэка невелик (уточнить alive/revoked вместо честного «не знаю»), а цена
промаха — оплаченный аккаунт без права восстановления.
...
def _listener_may_hold_sessions() -> bool:
    """Держит ли сейчас слушатель подключения к аккаунтам.

    Проверка живости идёт отдельным процессом и в память слушателя не заглянет, поэтому
    судим по тому же тумблеру, которым он сам себя останавливает (listener_enabled).
    Осторожная сторона: считаем, что держит, если тумблер включён."""
    try:
        with database.get_conn() as conn:
            return database.get_setting(conn, "listener_enabled", "on") != "off"
    except Exception:  # noqa: BLE001 — БД недоступна: считаем, что держит (безопаснее)
        return True
...
async def _check_one(acc: dict) -> tuple[str, str]:
    ...
    Прямая проба идёт с IP сервера и потому опасна, пока сессию держит слушатель: два
    разных IP на одном ключе = сожжённый аккаунт (подробно в шапке модуля). Когда
    слушатель включён, честно останавливаемся на 'noconn' — это статус «не знаю», он
    аккаунт не хоронит и из рассылки не выбрасывает."""
    state, reason = await _probe(acc, acc.get("proxy"))
    if state == "noconn" and acc.get("proxy"):
        if _listener_may_hold_sessions():
            return state, f"{reason}; напрямую не проверяли — слушатель держит сессию, " \
                          "вторая проба с IP сервера сожгла бы ключ"
