...
Боевой пульт на GCP: **http://34.12.128.70:8000/**. Это единственный адрес, где оператор проверяет и использует изменения; локальная машина — только исходники перед штатным деплоем. **Кнопка деплоя стирает любые ручные правки на сервере.** Изменения вносятся только локально → коммит → штатный деплой. Не редактировать код через SSH напрямую.

**Не коммитить на сервере.** У сервера нет доступа на push в GitHub, а деплой делает `reset --hard origin/main`: коммит, сделанный на сервере, живёт до первого деплоя и молча исчезает. 25.09.2026 так дважды разъехались правки двух сессий (исправление агента откатилось, крымский текст ушёл в чужую кампанию). Если на сервере всё же появились коммиты (`git status` → `ahead N`), сначала забери их локально и запушь: `GIT_SSH_COMMAND="ssh -i ~/.ssh/axiom_claude" git fetch ssh://vassiliy_pelts@34.12.128.70/home/vassiliy_pelts/axiom-repo main && git merge --ff-only FETCH_HEAD && git push origin main`, и только потом деплой.

## Кампании

- Ремонт Сочи (9410): состояние, решения и что осталось сделать — `docs/CAMPAIGN_9410_REMONT_SOCHI.md`.
